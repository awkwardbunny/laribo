...
(function() {
    ...
    function build(opts) {
        ...
        svg.Font = new(function() {
            ...
            this.Parse = function(s) {
                ...
                for (var i = 0; i < d.length; i++) {
                    if (!set.fontStyle && that.Styles.indexOf(d[i]) != -1) { if (d[i] != 'inherit') f.fontStyle = d[i];
                        set.fontStyle = true; } else if (!set.fontVariant && that.Variants.indexOf(d[i]) != -1) { if (d[i] != 'inherit') f.fontVariant = d[i];
                        set.fontStyle = set.fontVariant = true; } else if (!set.fontWeight && that.Weights.indexOf(d[i]) != -1) { if (d[i] != 'inherit') f.fontWeight = d[i];
                        set.fontStyle = set.fontVariant = set.fontWeight = true; } else if (!set.fontSize) { if (d[i] != 'inherit') f.fontSize = d[i].split('/')[0];
                        set.fontStyle = set.fontVariant = set.fontWeight = set.fontSize = true; } else { if (d[i] != 'inherit') ff += d[i]; }
                }
        ...
        svg.AspectRatio = function(ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
            ...
            if (meetOrSlice == 'meet') { desiredWidth *= scaleMin;
                desiredHeight *= scaleMin; }
            if (meetOrSlice == 'slice') { desiredWidth *= scaleMax;
        ...
        svg.Element.ElementBase = function(node) {
            ...
            this.attribute = function(name, createIfNotExists) {
                ...
                if (createIfNotExists == true) { a = new svg.Property(name, '');
            ...
            this.style = function(name, createIfNotExists, skipAncestors) {
                ...
                if (createIfNotExists == true) { s = new svg.Property(name, '');
                    this.styles[name] = s; }
        ...
        svg.Element.image = function(node) {
            ...
            if (!isSvg) {
                ...
                this.img.onerror = function() { svg.log('ERROR: image "' + href + '" not found');
                    self.loaded = true; }
                this.img.src = href;
            } else {
