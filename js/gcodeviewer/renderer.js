...
// ***** PRIVATE ******
    var canvas;
    var ctx;
    var zoomFactor= 4, zoomFactorDelta = 0.2;
    var gridSizeX=250,gridSizeY=210,gridStep=10;
...
//        colorLine: ["#000000", "#aabb88",  "#ffe7a0", "#6e7700", "#331a00", "#44ba97", "#08262f", "#db0e00", "#ff9977"],
    ...
    var drawGrid = function() {
        ...
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.moveTo(0, -gridSizeY*zoomFactor-offsetY);
        ctx.lineTo(gridSizeX*zoomFactor-offsetX, -gridSizeY*zoomFactor-offsetY);
        ctx.lineTo(gridSizeX*zoomFactor-offsetX, 0);
        ctx.lineTo(0, 0);
        ctx.lineTo(0, -gridSizeY*zoomFactor-offsetY);
        ctx.stroke();
...
//                ctx.moveTo(cmds[i].x*zoomFactor,cmds[i].y*zoomFactor);
                ...
                ctx.globalAlpha = cmds[i].alpha; // Set alpha for lines
                if(cmds[i].retract==0){
                    if(speedIndex>=0){
